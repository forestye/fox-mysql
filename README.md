...
- `yxmysql::SQLException`：基础 SQL 异常（包含错误码）
...
- `yxmysql_pool::PoolException`：连接池基础异常
- `yxmysql_pool::AcquireTimeoutException`：获取连接超时
- `yxmysql_pool::PoolShutdownException`：连接池已关闭
- `yxmysql_pool::HealthCheckException`：健康检查失败
...
```
yxmysql/
├── include/yxmysql/        # 公共头文件
│   ├── yxmysql.h          # 主头文件（包含所有核心功能）
│   ├── connection.h       # 连接类
│   ├── result_set.h       # 结果集类
│   ├── pool.h             # 连接池（独立命名空间 yxmysql_pool）
│   ├── exception.h        # 异常类型
│   └── types.h            # 类型定义
├── src/                   # 实现文件
├── examples/              # 示例代码
